 ...
 */
class createSite extends SController{
	 ...
	 * @param unknown_type $siteId
	 */
	public function createHomePage($siteId, $siteAlias){
		/*
		 //获取session帐户 信息
	 ...
	 * 生成栏目页
	 */
	public function createColumnPage($siteId, $siteAlias){
		/*
		 //获取session帐户 信息
		 $getSession = $this->getUtil("session");
		 $accountData = $getSession->get("accountData");
		 ...
		 */
		$config = SuperPHP::getConfig();
		$this->createColumnPageOp(0, "../" . $config['siteName']. "/" . (empty($this->createTwoDir)?"": $this->createTwoDir . "/") . $siteAlias, $siteId, $siteAlias, "");
	}
	/**
	 *
	 * 生成栏目页
	 */
	public function createColumnPageOp($parentId, $pathStr, $siteId, $siteAlias, $rPath){
		//获取栏目模板id
		$config = SuperPHP::getConfig();
		$contentClass = $this->_GetContentDao();// TMouldEngineManager::getContentClass();
		...
		foreach ($getCData[0] as $value){
			...
			if ($templateId > 0 && $columnAlias != "index"){
				...
				if(!empty($getArr)){
					...
					$templateFile = $config['siteTemplateDir'] . "/" . (empty($this->siteTemplateDir) ? $siteAlias : $this->siteTemplateDir) . "/" . $getTData[0][0]["template_file"];
	...
	public function createDetailPageOfColumnId($siteId, $columnId, $siteAlias, $cDataId = ""){
		...
		if (!empty($deTailTemplateId)){
			...
			if(!empty($getTData)){
				...
				$dao = new SDao();
				if(empty($targetAlias)){
					$sql = "select * from %s where column_id=%s and is_delete=0 and is_create=0";
					$sql = sprintf($sql, $mouldTable, $columnId);
				}else{
					$sql = "select * from %s where column_alias='%s' and is_delete=0 and is_create=0";
	 ...
	 * 生成被包含的模块页
	 */
	public function createIncludePage($templateFile, $siteAlias, $siteId=""){
		$config = SuperPHP::getConfig();
		...
		if(!empty($m)){
			foreach ($m[1] as $value){
				...
				$config = SuperPHP::getConfig();
				if (!empty($siteAlias)){
					$templateFileTmp = $config['siteTemplateDir'] . "/" . (empty($this->siteTemplateDir) ? $siteAlias : $this->siteTemplateDir) . "/" . $templateFileTmp;
				}
	...
	$commonThemePath, $parentColumnArr, $brotherColumnArr, $mouldId){
		...
		$str .= "}";
		//生成文件
		$pathStr = "../" . $siteMainName. "/" . (empty($this->createTwoDir)?"": $this->createTwoDir . "/") . $siteAlias . "/controller/tomms";
	 ...
	 */
	private function createPluginFile($siteId, $siteAlias){
		//全局配置
		$config = SuperPHP::getConfig();
		$str = "<?php \n";
		$str .= "\$p = \$_GET['p'];\n";
		$str .= "define('PLUGINPTAH', '" . $this->relativePath . $config['adminName'] . "/plugin/" . $this->moduleName . "/' . \$p . '/');\n";
		$str .= "require_once PLUGINPTAH . 'index.php';\n";
		//生成文件
		$targetPath = "../" . $config['siteName']. "/" . (empty($this->createTwoDir)?"": $this->createTwoDir . "/") . $siteAlias;
