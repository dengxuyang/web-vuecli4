...
class TTagRealizeAdapter extends SController implements ISTagAdapter{
	...
	public function compileTagForPlugin($module, $paramArr){
		...
		// $pluginStr .= "\$" . $dataResult . " = json_decode(substr(\$getData,3), true);\n";
	...
	private function sysStrReplace($str, $left, $right){
		...
		for($i=0; $i<count($m[1]); $i++){
			...
			$result = preg_replace("/".preg_quote($left). preg_quote($allStr) . preg_quote($right) ."/", $left . "include \"" . $templateFileTmp . "\"" . $right, $result);
