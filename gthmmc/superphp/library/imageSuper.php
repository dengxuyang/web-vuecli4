...
class imageSuper  {
	...
	*/
	public function cutImage($srcFile, $dstFile , $newWidth , $newHeight){
		$this->set_image($srcFile);
		$type = exif_imagetype($srcFile);
		$newimage = $this->get_data($newWidth, $newHeight);
		switch($type) {
		 case IMAGETYPE_JPEG :
		 	imagejpeg($newimage, $dstFile,100); // 存储图像
		 	break;
		 case IMAGETYPE_PNG :
		 	imagepng($newimage,$dstFile,100);
		 	break;
		 case IMAGETYPE_GIF :
		 	imagegif($newimage,$dstFile,100);
		 	break;
		 default:
		 	break;
		}
